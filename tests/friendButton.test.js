...
test('when user clicks on accept button, an axios post request is fired and button text changes to "End"', async () => {
    ...
    const { container } = render(<FriendButton id="3" />);
    const accept = await waitForElement(() =>
...
test('when button state is "Accept", a second "Cancel" button renders', async () => {
    axios.get.mockResolvedValue({
        data: {
            success: true,
            userId: 1,
...
test('when button state is not "Accept", a second "Cancel" button DOES NOT! render', async () => {
    axios.get.mockResolvedValue({
        data: {
            success: true,
            userId: 1,
    ...
    expect(friendBtn.innerHTML).toContain("End");
    const buttons = container.getElementsByTagName("button");
